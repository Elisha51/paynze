...
import { addSmsTemplate, updateSmsTemplate } from '@/services/templates';
...
const emptyTemplate: Partial<SmsTemplate> = {
    name: '',
    description: '',
    message: '',
}

export function SmsTemplateForm({ initialTemplate }: { initialTemplate?: SmsTemplate | null }) {
  const router = useRouter();
  const { toast } = useToast();
  const [template, setTemplate] = useState<Partial<SmsTemplate>>(initialTemplate || emptyTemplate);
  ...
  const handleSave = async () => {
    ...
    if (isEditing) {
        await updateSmsTemplate(template as SmsTemplate);
    } else {
        await addSmsTemplate(template as Omit<SmsTemplate, 'id'>);
    }
    
    toast({
        title: isEditing ? 'Template Updated!' : 'Template Created!',
        description: `The SMS template "${template?.name}" has been saved.`
    });
