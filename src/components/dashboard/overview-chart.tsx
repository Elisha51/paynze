...
export function OverviewChart({ orders, currency }: { orders: Order[], currency: string }) {
  const salesData = useMemo(() => {
    ...
    if (diffDays <= 31) { // Group by day
      ...
      return days.map(day => {
        const total = orders
          .filter(order => new Date(order.date).toDateString() === day.toDateString() && order.payment && order.payment.status === 'completed')
      ...
      return months.map(month => {
        ...
        const total = orders
          .filter(order => {
            ...
            return order.payment && order.payment.status === 'completed' && orderDate >= monthStart && orderDate <= monthEnd;
          })
          .reduce((sum, order) => sum + order.total, 0);
